...
Программное изделие является динамическим веб-приложением "3D визуализация учебного оборудования". Оно состоит из публичной части сайта, административной панели, серверного API и хранилища данных SQLite.

Сайт является динамическим, потому что содержимое каталога не зашито только в HTML-страницы: данные загружаются через API Express из SQLite-базы, карточки оборудования формируются в браузере на основе ответа сервера, новые модели можно добавлять через админ-панель, а QR-ссылки и журнал сканирования создаются во время работы приложения.
...
По результатам обследования было установлено, что система должна быть простой в эксплуатации и не требовать сложной серверной инфраструктуры. Для учебного проекта достаточно одного Node.js-сервера, SQLite-базы данных и клиентских HTML/CSS/JS-файлов, которые обеспечивают динамическую работу через REST API.
...
| Express | HTTP-сервер и маршрутизация | Простая реализация динамического REST API и выдачи клиентских файлов |
