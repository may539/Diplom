...
### QR-коды и просмотр с телефона

QR-код ведёт на `/view.html?id=…&scan=1` — полноэкранный просмотр модели на мобильном.

**Важно:** ссылка в QR должна быть доступна с телефона в той же Wi‑Fi сети. Адреса
`localhost`, `127.0.0.1` и внутренние IP Docker (`172.x.x.x`) с телефона не открываются.

1. Узнайте IP компьютера в локальной сети (например `192.168.0.42`).
2. Задайте в `.env`:
   ```env
   PUBLIC_BASE_URL=http://192.168.0.42:8080
   ```
3. Перезапустите контейнер: `docker compose up -d --build`.
4. Откройте админ-панель **по этому IP**, а не через `localhost`, и создайте/распечатайте QR заново.

Проверка настроек: `GET /api/server-info` — показывает предупреждение, если QR недоступен с телефона.
